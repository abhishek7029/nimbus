...
module "ec2_instance" {
  ...
  vpc_security_group_ids  = [data.terraform_remote_state.security_group.outputs.jenkins_sg]
  iam_instance_profile    = data.terraform_remote_state.iam_role.outputs.jenkins_profile_name
  subnet_id               = data.aws_subnets.app-tools.ids[0]
